...
async def _get_category_id(auth_client: AsyncClient) -> str:
...
class TestShareSkillApi:
    ...
    async def test_should_return_201_when_share_skill_given_valid_skill_and_category(
        ...
        category_id = await _get_category_id(auth_client)
        ...
        assert data["snapshot_name"] == name
        assert data["snapshot_description"] == skill["description"]
